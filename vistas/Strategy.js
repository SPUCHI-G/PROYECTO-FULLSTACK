...
.then(response => {
    ...
    form.addEventListener('submit',(ev)=>{
        ev.preventDefault(); 
        const template = (titulo, imagen, descr,url,genre) => {
        return `
            <div class="col mb-3">
                <div class="card">
        ...
        const crearcard = (res,divResult,nom) => {
            if(nom.value==''){
                ...
                for(let i = 0 ; i<res.length ; i++){
                    if(res[i].genre=="Strategy"){
                    columnas += template(res[i].title,res[i].thumbnail,res[i].short_description,res[i].game_url,res[i].genre);
                    }
                    if(cont === 4){
